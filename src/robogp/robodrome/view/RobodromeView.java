...
public class RobodromeView extends JComponent {
    ...
    public Direction getRobotDirection(RobotMarker robot){
        return  getMarker(robot).getDirection();
    }
    
    public int xToCol(int x){
        int col = (x+originX-BORDER)/cellSize;
        return (col >= drome.getColumnCount() || col < 0) ? -1 : col;
    }
    
    public int yToRow(int y){
        int row = (y+originY-BORDER)/cellSize;
        return (row >= drome.getRowCount() || row < 0) ? -1 : row;
    }
    
    public int getRowCount(){
        return drome.getRowCount();
    }
    
    public int getColCount(){
        return drome.getColumnCount();
    }
      
    public int getRobotRowPos (RobotMarker robot){
        return getMarker(robot).getRowPos();
    }
    
    public int getRobotColPos (RobotMarker robot){
        return getMarker(robot).getColPos();
    }
    ...
    //Restituisce il tipo di cella di una determinata coordinata
    public char getTypeCell(int row, int col){
        if (drome.getCell(row, col) == null)
            return 'X';
        else
            return drome.getCell(row, col).getType();
    }
    
    //Restituisce true se la cella ha un muro nella direzione indicata
    public boolean wallOnCell(int row, int col, Direction dir){
        if (drome != null && drome.getCell(row,col) == null)    
            return false;
        else
            return drome.getCell(row,col).hasWall(dir);
    }
    
    //posiziona il robot sul' "n"-esimo dock di partenza

    /**
     *
     * @param robot
     */
    public void put_robot_in_dock(RobotMarker robot){
        //java.awt.EventQueue.invokeLater(new Runnable() {
            //@Override
            //public void run() {
                FloorCell temp_dock = drome.getDock(robot.getDock());
                placeRobot(robot, temp_dock.getDockDirection(), temp_dock.get_dock_r(), temp_dock.get_dock_c(), true);
            //}
        //});
    }
    
    public int get_checkpoint(RobotMarker robot){
        int check = 0;
        BoardCell cell = (BoardCell)drome.getCell(this.getRobotRowPos(robot),this.getRobotColPos(robot));
        if (cell != null && cell.getType() == 'F'){
            FloorCell floor_cell = (FloorCell)cell;
            if (floor_cell.isCheckpoint())
                check = floor_cell.getCheckpoint();
        }
        return check;        
    }
    
    public int get_total_checkpoints(){
        return drome.getTotCheck();
    }
    
    public boolean onRepOrUpd(RobotMarker robot){
        BoardCell cell = (BoardCell)drome.getCell(this.getRobotRowPos(robot),this.getRobotColPos(robot));
        if (cell.getType() == 'F'){
            if ( ((FloorCell)cell).isRepair() || ((FloorCell)cell).isUpgrade())
                return true;
        }
        return false;
    }
    
    public void cell_rotation_robot(RobotMarker robot){
        FloorCell rot_cell = (FloorCell)drome.getCell(this.getRobotRowPos(robot),this.getRobotColPos(robot));
        if (rot_cell.isLeftRotator())
            addRobotMove(robot, 0, this.getRobotDirection(robot), Rotation.CCW90);
        if (rot_cell.isRightRotator())
            addRobotMove(robot, 0, this.getRobotDirection(robot), Rotation.CW90);
    }
    
    //Gestisce le animazioni del nastro per il robot
    public void belt_robot(RobotMarker robot){
        BeltCell cell = (BeltCell)drome.getCell(this.getRobotRowPos(robot),this.getRobotColPos(robot));
        if (cell.isTurnLeft()){
            addRobotMove(robot, 1, cell.getOutputDirection(), Rotation.getCounterClockwiseRotation(this.getRobotDirection(robot), cell.getOutputDirection()));
            return;
        }
        if (cell.isTurnRight()){
            addRobotMove(robot, 1, cell.getOutputDirection(), Rotation.getClockwiseRotation(this.getRobotDirection(robot), cell.getOutputDirection()));
            return;
        }
        addRobotMove(robot, 1, cell.getOutputDirection(), Rotation.NO);
    }
    
    //overloading del metodo nel caso sia necessaria la spinta di altri robot
    public void belt_robot(RobotMarker robot, RobotMarker[] push){
        BeltCell cell = (BeltCell)drome.getCell(this.getRobotRowPos(robot),this.getRobotColPos(robot));
        if (cell.isTurnLeft()){
            addRobotMove(robot, 1, cell.getOutputDirection(), Rotation.getCounterClockwiseRotation(this.getRobotDirection(robot), cell.getOutputDirection()), push);
            return;
        }
        if (cell.isTurnRight()){
            addRobotMove(robot, 1, cell.getOutputDirection(), Rotation.getClockwiseRotation(this.getRobotDirection(robot), cell.getOutputDirection()), push);
            return;
        }
        addRobotMove(robot, 1, cell.getOutputDirection(), Rotation.NO, push);
    }
    
    public Direction beltDirection(RobotMarker robot){
        BeltCell cell = (BeltCell)drome.getCell(this.getRobotRowPos(robot),this.getRobotColPos(robot));
        return cell.getOutputDirection();
    }
    ...
    //Verifica se almeno uno tra tutti i robot in pista è nelle cordinate date
    public RobotMarker robotNear(int row, int col, ArrayList<RobotMarker> robots){
        for (RobotMarker robot: robots){
            if ( (row == getMarker(robot).getRowPos()) && (col == getMarker(robot).getColPos()) )
                return robot;
        }
        return null;
    }
    ...
    public boolean robotVisible(RobotMarker robot){
        return getMarker(robot).isVisible();
    }
